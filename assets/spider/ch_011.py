...
class Spider(Spider):
    def init(self, extend=None):
        ...
        self.session.headers.update({'User-Agent': UA, 'apikey': KEY, 'Authorization': 'Bearer ' + KEY})
        self.base = HOST
    ...
    def _call(self, name, body=None, query=''):
        ...
        def go(base):
            try:
                ...
                if body is not None:
                    r = self._s().post(u, json=body, timeout=10)
                else:
                    r = self._s().get(u, timeout=10)
        ...
        bases = [self.base, SUP] if self.base != SUP else [SUP]
        with ThreadPoolExecutor(max_workers=2) as ex:
            futs = [ex.submit(go, b) for b in bases]
            for f in as_completed(futs):
                d = f.result()
                if d:
                    self.base = futs.index(f) and SUP or self.base
                    return d
        return None

    def _feed(self, deep=8):
        if getattr(self, '_feed_cache', None):
    ...
    def _items(self, d, tag=None):
        ...
        for it in d.get('items') or []:
            ...
            rows.append({'vod_id': vid, 'vod_name': title, 'vod_pic': self._pic(it), 'vod_remarks': self._rm(it)})
        return rows
    ...
    def _series_items(self, d):
        ...
        for s in d.get('series') or []:
            ...
            rows.append({'vod_id': 'b:' + s.get('bundle_id', ''), 'vod_name': s.get('title') or '', 'vod_pic': s.get('cover_url') or '', 'vod_remarks': remark})
        return rows
    ...
    def categoryContent(self, tid, pg, filter=False, extend=None):
        ...
        if tid == 'series':
            ...
            return {'list': self._series_items(d), 'page': pg, 'pagecount': max(1, total // 20 + 1)}
        if tid == 'feed' or not tid:
            d = self._call('content-feed', query='limit=20')
            return {'list': self._items(d) if d else [], 'page': pg, 'pagecount': 1}
        label = tid[3:] if tid.startswith('rt:') else tid
        return {'list': self._items({'items': self._feed()}, label), 'page': pg, 'pagecount': 1}
